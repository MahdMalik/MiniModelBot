...
vector<float> DenseLayer::forward(const vector<float>& input, bool isLastLayer){
    ...
    for (int o = 0; o < outputSize; o++) {
        ...
        out[o] = isLastLayer ? sigmoid(sum) : relu(sum);
    }
    return out;
}
...
void CustomHead::init(int featureSize){
    // layer1 loads its weights from the headed model's last layer
    layer1.initFromLoadedModel(featureSize, 32);
    layer2.initRandom(32, 2);

    // Try loading from NVS — overwrites random init if saved weights exist
